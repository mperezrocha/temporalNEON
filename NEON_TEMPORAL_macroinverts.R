...
# to get to the  genus and finer resolution further 
...
#it's easier to keep simple like this in order to get one entry per site per row (species per sites summed)
...
inv_dat_aggregate_m <- inv_dat_fine %>%
  ...
  summarise(
    abundance = sum(den), .groups = 'drop') %>% 
  ungroup()


View(inv_dat_aggregate_m)


###put in the format required for codyn and BAT analyses
...
# make wide first, filling with abundance, making the 'bout' column (format year+month) 
inv_dat_wide <- inv_dat_aggregate_m %>%
